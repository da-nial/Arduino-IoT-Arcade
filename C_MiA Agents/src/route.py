...
def authenticate_agent(agent: Agent):
    agents_ids = [a['id'] for a in VALID_AGENT_WITH_THINGS]
    return agent.ident in agents_ids


def authenticate_things(agent_id, device_id):
    for a in VALID_AGENT_WITH_THINGS:
        if a['id'] == agent_id:
            if device_id in a['things']:
                return True
    return False
...
class Handler():
    ...
    def on_log(self, _client, _userdata, message):
        ...
        try:
            ...
            if not authenticate_things(log.agent, log.device):
    ...
    def on_discovery(self, client, userdata, message):
        ...
        try:
            ...
            if not authenticate_agent(agent):
